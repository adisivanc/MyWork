...
(function ($, window, document) {
    ...
    function Plugin(el, options) {
        ...
        elem.on('ett-tltp:show ' + options.trigger.show, function(){
            $.data(this, 'cancel.ett-tltp', true);

            var tip = $.data(this, 'active.ett-tltp');
            if (!tip) {
                tip = $('<div class="ett-tltp"><div class="ett-tltp-inner"/></div>').css({position:'absolute', zIndex:100000});
                $.data(this, 'active.ett-tltp', tip);
            }
            ...
            tip.find('.ett-tltp-inner')[options.html ? 'html' : 'text'](title || options.fallback);

            var pos = $.extend({}, elem.offset(), {width: this.offsetWidth, height: this.offsetHeight});

            tip[0].className = 'ett-tltp';
            tip.remove().css({
                top: 0,
                left: 0,
                opacity: 0
            ...
            switch (options.align.charAt(0) === 'a' ? dir : options.align.charAt(0)) {
                case 'b':
                    tip.css({top: pos.top + pos.height, left: pos.left + pos.width / 2 - actualWidth / 2}).addClass('ett-tltp-bottom');
                    break;
                case 't':
                    tip.css({top: pos.top - actualHeight, left: pos.left + pos.width / 2 - actualWidth / 2}).addClass('ett-tltp-top');
                    break;
                case 'l':
                    tip.css({top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left - actualWidth}).addClass('ett-tltp-left');
                    break;
                case 'r':
                    tip.css({top: pos.top + pos.height / 2 - actualHeight / 2, left: pos.left + pos.width}).addClass('ett-tltp-right');
                    break;
            }
        ...
        elem.on('ett-tltp:hide ' + options.trigger.hide, function(){
            $.data(this, 'cancel.ett-tltp', false);
            var self = this;
            timeout = setTimeout(function () {
                if ($.data(self, 'cancel.ett-tltp')) return;
                var tip = $.data(self, 'active.ett-tltp');
                if (options.fade) {
